...
WHERE DEPT_CODE IN ('D9', 'D5')
    AND HIRE_DATE < TO_DATE(20020101);

-- 11. EMPLOYEE테이블에 고용일이 90/01/01 ~ 01/01/01인 사원의 전체 내용을 조회
SELECT * 
FROM EMPLOYEE
WHERE HIRE_DATE BETWEEN TO_DATE(19900101) AND TO_DATE(20010101);
...
WHERE INSTR(EMAIL,'_') -1 = 4
    ...
    AND HIRE_DATE BETWEEN TO_DATE(19900101) AND TO_DATE(20001201)
    AND SALARY >= 2700000;
    
-- 15. EMPLOYEE테이블에서 사원 명과 직원의 주민번호를 이용하여 생년, 생월, 생일 조회
--SELECT EMP_NAME, SUBSTR(EMP_NO,1,6), SUBSTR(EMP_NO,3,2), SUBSTR(EMP_NO,5,2)
...
SELECT EMP_NAME, SUBSTR(EMP_NO,1,6), SUBSTR(EMP_NO,8,14), 
    TO_NUMBER(SUBSTR(EMP_NO,1,6)) + TO_NUMBER(SUBSTR(EMP_NO,8,14))
FROM EMPLOYEE
WHERE EMP_ID = 201;
...
*/
SELECT EXTRACT(YEAR FROM HIRE_DATE) AS 연도, COUNT(*)
FROM EMPLOYEE
...
SELECT COUNT(*) AS "전체 직원 수",
    SUM(CASE 
        ...
        END) AS "2001년",
    SUM(CASE 
        WHEN EXTRACT(YEAR FROM HIRE_DATE) = 2002 
        THEN 1 ELSE 0 
        END) AS "2002년",
    SUM(CASE
        WHEN EXTRACT(YEAR FROM HIRE_DATE) = 2003 
        THEN 1 ELSE 0 
        END) AS "2003년",
    SUM(CASE 
        WHEN EXTRACT(YEAR FROM HIRE_DATE) = 2004 
        THEN 1 ELSE 0 
        END) AS "2004년"
FROM EMPLOYEE;
