/*
    ...
    임시 테이블 같은 존재(실제 데이터가 담겨있는건 아니다. => 논리적인 테이블)
*/
...
-- 뷰를 통해 조작하게되면 실제 데이터가 담겨있는 테이블에 반영이 된다.

CREATE OR REPLACE VIEW VW_JOB
AS SELECT JOB_CODE, JOB_NAME
    FROM JOB;
    
SELECT * FROM VW_JOB; --논리테이블 (실제 데이터 X)
SELECT * FROM JOB;
...
/*
    ...
    2) 뷰에 정의되어있지 않은 컬럼 중 베이스 테이블 상에 NOT NULL 제약조건이 지정되어있는 경우
    3) 산술연산식 또는 함수식으로 정의가 되어있는 경우
    4) 그룹함수나 GROUP BY 절이 포함된 경우
    5) DISTINCT구문이 포함된 경우
    6) JOIN을 이용해 여러 테이블을 연결시켜놓은 경우

    대부분 뷰는 조회를 목적으로 생성한 것이다.
    그냥 뷰에서 DML을 쓰지 말자
*/
...
/*
    ...
    2) FORCE | NOFORCE
        ...
        - NOFORCE(기본옵션): 서브쿼리에 기술된 테이블이 존재하는 테이블이여야만 뷰가 생성되게 하는 옵션
    3) WITH CHECK OPTION: DML 시 서브쿼리에 기술된 조건에 부합한 값으로만 DML이 가능하도록 해라
    4) WITH READ ONLY: 뷰에 대해서 조회만 가능하도록 해라
...
-- 서브쿼리에 기술된 테이블이 존재하지 않아도 뷰가 우선 생기게 된다.
CREATE OR REPLACE FORCE VIEW VW_EMP -- 뷰를 먼저 만들어놓겠다 라는 행동
AS SELECT TCODE, TNAME, TCONTENT
    FROM TT;
...
CREATE TABLE TT(
    TCODE NUMBER, TNAME VARCHAR2(20), TCONTENT VARCHAR2(30)
);
...
-- 200번 사원 급여를 200만원으로 변경(SALARY >= 3000000 조건에 맞지 않음)
UPDATE VW_EMP
SET SALARY = 2000000
WHERE EMP_ID = 200;
...
-- 200번 사원 급여를 200만원으로 변경(SALARY >= 3000000 조건에 맞지 않음)이 되지 않는다.
UPDATE VW_EMP
SET SALARY = 2000000
WHERE EMP_ID = 200;
