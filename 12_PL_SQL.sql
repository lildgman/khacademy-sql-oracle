...
DECLARE
    EID NUMBER;
    ENAME VARCHAR2(20);
    PI CONSTANT NUMBER := 3.14; -- 상수 , := 가 대입 연산자
BEGIN
    EID := &번호ㅇㅇ;
    ENAME := '&ㅇㅇ이름';
...
/*
    ...
    사용자가 입력한 사번의 사원의 사번, 직급코드, 급여, 부서명 조회 후 각 변수에 담아 출력
*/
...
-- 보너스를 받지 않은 사원은 보너스율 출력 전 '보너스를 지급받지 않은 사원입니다 출력'
DECLARE
    EID EMPLOYEE.EMP_ID%TYPE;
    ENAME EMPLOYEE.EMP_NAME%TYPE;
    SAL EMPLOYEE.SALARY%TYPE;
...
/
--------------------------------------------------------------------------
--4) CASE 비교대상자 WHEN 동긍비교값1 THEN 결과값 WHEN 비교값2 THEN 결과값... ELSE 결과값 END;
DECLARE
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE(EMP.EMP_NAME ||'사원은 '|| DNAME ||'입니다.');
...
/*
    ...
    EXCEPTION
        ...
        *시스템 예외(오라클에서 미리 정의해둔 예외)
        ...
        - DUF_VAR_ON_INDEX: UNIQUE 제약조건에 위배되었을 경우
        ...
