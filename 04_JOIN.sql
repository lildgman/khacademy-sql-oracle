/*
    ...
    JOIN은 크게 오라클 전용 구문과 "ANSI 구문"(ANSI == 미국국립표준협회)
    ...
    ----------------------------------------------------------------------
        ...
        (EQUAL JOIN)                 |           JOIN USING/ON
    ---------------------------------------------------------------------
          포괄조인                      |         왼쪽 외부 조인(LEFT OUTER JOIN)
         LEFT OUTER                   |         오른족 외부 조인(RIGHT OUTER JOIN)
...
/*
    ...
    연결시키는 컬럼의 값이 일치하는 행들만 조회(==일치하는 값이 없는 행 조회 제외)
*/
...
ORDER BY DEPT_CODE;
-- NULL, D3, D4, D7 데이터는 한쪽 테이블에서만 존재하기 때문에 제외된 걸 확인할 수 있다.
...
--JOIN USING 같은 경우 연결하는 컬럼명이 같을 때만 사용가능
...
-- 사원명, 부서명, 급여, 연봉
-- 내부조인 시 부서배치를 받지 않는 2명의 사원정보 누락
SELECT *
FROM EMPLOYEE
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID);

-- 1) LEFT JOIN: 두 테이블 중 왼편에 기술된 테이블을 기준으로 조인
-- 오라클
...
-- 3) FULL [OUTER] JOIN: 두 테이블이 가진 모든 행 조회 가능 (ANSI만 있음)
-- ANSI
SELECT EMP_NAME, DEPT_TITLE, SALARY, SALARY*12
FROM EMPLOYEE
...
-- 전체 사원들의 사원사번, 사원명, 사원부서코드, 
--            사수사번, 사수명, 사수부서코드
--> 오라클
SELECT E.EMP_ID AS 사원사번, E.EMP_NAME AS 사원명, E.DEPT_CODE,
    M.EMP_ID AS 사수사번, M.EMP_NAME AS 사수사원명, M.DEPT_CODE
